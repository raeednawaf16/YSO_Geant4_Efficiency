...
void ggmatrix() {
    TFile *inFile = TFile::Open("hist1240-00_44.root", "READ"); // Use your actual file name
    if (!inFile || inFile->IsZombie()) {
        std::cout << "Error: Cannot open input file." << std::endl;
        return;
    }
    TH2F *h2 = (TH2F*)inFile->Get("hpge/aa"); // Use your actual TH2 name
    if (!h2) {
    ...
    h1_clean_gate->SetTitle("1173 keV region gated on 1332 keV");
    h1_clean_gate->Add(h1_bg, -scaleFactor);

    // Save the resulting gated 1D spectrum
    TFile *outFile = TFile::Open("addback_1173_gated_1332_44.root", "RECREATE");
    h1_clean_gate->Write();
